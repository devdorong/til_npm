...
function App() {
  ...
  useEffect(() => {
    ...
    // 스크립트가 로드가 완려되면 실행될 이벤트 리스너를 추가한다.
    kakaoMapscript.onload = () => {
      // console.log(window.kakao);
      window.kakao.maps.load(() => {
        const contatiner = document.getElementById("map");
        const options = {
          center: new window.kakao.maps.LatLng(37.5665, 126.978),
          level: 3,
        };
        const map = new window.kakao.maps.Map(contatiner, options);
...
- 레퍼런스 : https://www.npmjs.com/package/react-kakao-maps-sdk
- 참조블로그: https://velog.io/@wlwl99/React-Kakao-Map-SDK-사용하기
- 설치 : `npm i react-kakao-maps-sdk`
...
function App() {
  ...
  const [isMpaloaded, setIsMaploaded] = useState(false);
    ...
    <div>
      ...
      <div>
        ...
        >
          ...
          <CustomButton onClick={() => setLevel(level + 1)}>축소</CustomButton>
          <CustomButton onClick={() => setLevel(level - 1)}>확대</CustomButton>
        </Map>
      </div>
    </div>
  );
